...
def get_timebound_patient_icustay(Patient_ICUstay, past_time=0, start_hr = None, end_hr = None, include_end_hr=False):
    # Inputs:
    ...
    if not (end_hr == None):
        if include_end_hr:
            ...
            #Patient_ICUstay.cxr = Patient_ICUstay.cxr[(Patient_ICUstay.cxr.deltacharttime <= end_hr)]
        else:
            Patient_ICUstay.emar = Patient_ICUstay.emar[(Patient_ICUstay.emar.deltacharttime < end_hr-past_time) | pd.isnull(Patient_ICUstay.emar.deltacharttime)]
            Patient_ICUstay.labevents = Patient_ICUstay.labevents[(Patient_ICUstay.labevents.deltacharttime < end_hr-past_time) | pd.isnull(Patient_ICUstay.labevents.deltacharttime)]
            Patient_ICUstay.microbiologyevents = Patient_ICUstay.microbiologyevents[(Patient_ICUstay.microbiologyevents.deltacharttime < end_hr-past_time) | pd.isnull(Patient_ICUstay.microbiologyevents.deltacharttime)]
            Patient_ICUstay.outputevents = Patient_ICUstay.outputevents[(Patient_ICUstay.outputevents.deltacharttime < end_hr-past_time) | pd.isnull(Patient_ICUstay.outputevents.deltacharttime)]
            Patient_ICUstay.datetimeevents = Patient_ICUstay.datetimeevents[(Patient_ICUstay.datetimeevents.deltacharttime < end_hr-past_time) | pd.isnull(Patient_ICUstay.datetimeevents.deltacharttime)]
            Patient_ICUstay.chartevents = Patient_ICUstay.chartevents[(Patient_ICUstay.chartevents.deltacharttime < end_hr-past_time) | pd.isnull(Patient_ICUstay.chartevents.deltacharttime)]
            Patient_ICUstay.cxr = Patient_ICUstay.cxr[(Patient_ICUstay.cxr.deltacharttime < end_hr-past_time) | pd.isnull(Patient_ICUstay.cxr.deltacharttime)]
            Patient_ICUstay.imcxr = [Patient_ICUstay.imcxr[i] for i, x in enumerate((Patient_ICUstay.cxr.deltacharttime < end_hr-past_time) | pd.isnull(Patient_ICUstay.cxr.deltacharttime)) if x]
            #Notes
            #Patient_ICUstay.noteevents = Patient_ICUstay.noteevents[(Patient_ICUstay.noteevents.deltacharttime <= end_hr) | pd.isnull(Patient_ICUstay.noteevents.deltacharttime)]
            Patient_ICUstay.dsnotes = Patient_ICUstay.dsnotes[(Patient_ICUstay.dsnotes.deltacharttime <end_hr-past_time) | pd.isnull(Patient_ICUstay.dsnotes.deltacharttime)]
            #Patient_ICUstay.ecgnotes = Patient_ICUstay.ecgnotes[(Patient_ICUstay.ecgnotes.deltacharttime <= end_hr) | pd.isnull(Patient_ICUstay.ecgnotes.deltacharttime)]
            #Patient_ICUstay.echonotes = Patient_ICUstay.echonotes[(Patient_ICUstay.echonotes.deltacharttime <= end_hr) | pd.isnull(Patient_ICUstay.echonotes.deltacharttime)]
            Patient_ICUstay.radnotes = Patient_ICUstay.radnotes[(Patient_ICUstay.radnotes.deltacharttime < end_hr-past_time) | pd.isnull(Patient_ICUstay.radnotes.deltacharttime)]
            #Patient_ICUstay.cxr = Patient_ICUstay.cxr[(Patient_ICUstay.cxr.deltacharttime < end_hr-past_time) | pd.isnull(Patient_ICUstay.cxr.deltacharttime)]

        # Filter CXR to match allowable patient stay (always include image in current timeframe)
        # if include_end_hr:
...
def is_haim_patient_keyword_match(patient, keywords, verbose = 0):
    ...
    patient_dfs_dict = ['core', 'diagnoses_icd', 'drgcodes', 'emar', 'emar_detail', 'hcpcsevents', 'labevents', 'microbiologyevents', 'poe',
                        ...
                        'datetimeevents', 'chartevents', 'cxr', 'imcxr', 'dsnotes', 'radnotes']
                        
                       # 'noteevents', 'dsnotes', 'ecgnotes', 'echonotes', 'radnotes']
